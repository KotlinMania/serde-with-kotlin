...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `SerdeWith.swiftmodule/`
...
//      `import SerdeWith` above would fail at compile time.
//
//   2. The static archive `libSerdeWith.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "SerdeWith" }` and
//      `flattenPackage = "io.github.kotlinmania.serdewith"` configuration in
...
//      via the `SerdeWithLibrary` product.
//
...
final class SerdeWithExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "SerdeWith swift module imported cleanly")
    }
}
