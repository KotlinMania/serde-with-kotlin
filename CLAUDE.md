# Claude Code Project Instructions — serde-with-kotlin

## Project Overview

This is **serde-with-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`serde_with`](https://crates.io/crates/serde_with).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/serde_with/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/serde_with>
